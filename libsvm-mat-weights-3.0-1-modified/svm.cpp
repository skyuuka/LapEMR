...
class LapSVR_Q: public Kernel
{	
public:
	LapSVR_Q(const svm_problem& prob, const svm_parameter& param)
	:Kernel(prob.l, prob.x, param)
	{
		this->prob = &prob;
		l = prob.l;
		...
		for(int k=0;k<l;k++)
		{
			...
			index[k+l] = k;
			//QD[k] = (this->*kernel_function)(k,k);
			QD[k] = extend_kernel_function(k,k);
			QD[k+l] =QD[k];
		}
		buffer[0] = new Qfloat[2*l];
		buffer[1] = new Qfloat[2*l];
	...
	Qfloat extend_kernel_function(int i, int j) const
	{
		double kv = (this->*kernel_function)(i,j);			
		double Uij = (this->kernel_linear2)(i,j);
		return (Qfloat)(kv + Uij);
	}

    double kernel_linear2(int i, int j)  const
    {
		return dot(prob->x2[i],prob->x2[j]);
    }
	...
	Qfloat *get_Q(int i, int len) const
	{
		...
		if(cache->get_data(real_i,&data,l) < l)
		{
			for(j=0;j<l;j++)
				//data[j] = (Qfloat)(this->*kernel_function)(real_i,j);
				data[j] = (Qfloat)extend_kernel_function(real_i,j);
		}
...
static void solve_laplacian_svr(
	const svm_problem *prob, const svm_parameter *param,
	double *alpha, Solver::SolutionInfo* si)
{
	int l = prob->l;
	...
	Solver s;
	s.Solve(2*l, LapSVR_Q(*prob,*param), linear_term, y,
		alpha2, C, param->eps, si, param->shrinking);
	double sum_alpha = 0;
	for(i=0;i<l;i++)
	{
	...
	double Cp, double Cn)
{
	...
	switch(param->svm_type)
	{
		...
		case LapSVR:
			si.upper_bound = Malloc(double, 2*prob->l);
			solve_laplacian_svr(prob, param, alpha, &si);
			break;
	}
