...
void save_problem2(const char *filename, const struct svm_problem  &prob)
{
    ...
    for(int i=0; i < prob.l; i++)
    {
        ...
        for( int j=0; prob.x2[i][j].index != -1; j++)
        {
            fprintf(fp, " %d:%f", prob.x2[i][j].index, prob.x2[i][j].value);
        }
        fprintf(fp,"\n");
    }
...
void save_problem1(const char *filename, const struct svm_problem  &prob)
{
    FILE *fp=fopen(filename, "w");
    if (!fp)
    {
...
// read in a problem (in svmlight format)
int read_problem_dense2(const mxArray *instance_mat)
{
	int i, j, k;
	int elements, sc;
	double *samples;
	...
	prob.x2 = Malloc(struct svm_node *,prob.l);
	x2_space = Malloc(struct svm_node, elements);

	j = 0;
	for(i = 0; i < prob.l; i++)
	{
		prob.x2[i] = &x2_space[j];
		for(k = 0; k < sc; k++)
		{
			if(samples[k * prob.l + i] != 0)
			{
				x2_space[j].index = k + 1;
				x2_space[j].value = samples[k * prob.l + i];
				j++;
			}
		}
		x2_space[j++].index = -1;
...
int read_problem_sparse2(const mxArray *instance_mat)
{
	int i, j, k, low, high;
	mwIndex *ir, *jc;
	...
	for(i=0;i<prob.l;i++)
	{
		...
		for(k=low;k<high;k++)
		{
			x2_space[j].index = (int)ir[k] + 1;
			x2_space[j].value = samples[k];
			j++;
		}
		x2_space[j++].index = -1;
	}
		...
		int nrhs, const mxArray *prhs[] )
{
	...
	if(nrhs > 2 && nrhs < 6)
	{
		...
		if (param.svm_type == LapSVR)
		{
			if (mxIsSparse(prhs[4]))
			{
				err = read_problem_sparse2(prhs[4]);
			} 
			else
			{
                err = read_problem_dense2(prhs[4]);
			}
            //save_problem2("t2.txt", prob);
		}

        //save_problem1("t1.txt", prob);
